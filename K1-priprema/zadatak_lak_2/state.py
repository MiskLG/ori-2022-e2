...
class RobotState(State):
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        if self.board.data[self.position[0]][self.position[1]] == 'bb' and self.blue_box == 0:
            self.need = True
            self.blue_box += 1
            self.eaten_boxes.append([self.position[0], self.position[1]])
        if self.board.data[self.position[0]][self.position[1]] == 'ob' and self.blue_box == 1 and self.orange_box == 0:
            self.need = True
            self.orange_box += 1
            self.eaten_boxes.append([self.position[0], self.position[1]])
    ...
    def is_final_state(self):
        if self.need:
            return True, self.position == self.goal_position and self.orange_box == 1 and self.blue_box == 1
        return False, self.position == self.goal_position and self.orange_box == 1 and self.blue_box == 1
