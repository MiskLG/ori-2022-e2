...
class RobotState(State):
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        if (self.board.data[row][col] == 'l' or self.board.data[row][col] == 'og') and self.lost_points < 4 and self.points < 4:
            leave = False
    ...
    def get_legal_positions(self):
        ...
        data = self.get_legal_moves_vertical(self.board, self.position, ['.', 'l', 'og','g'], [], 1, self.board.cols, [True, True, True, True])
        data2 = self.get_legal_moves_diagonal(self.board, self.position, ['.', 'l', 'og','g'], [], 1, self.board.cols, [True, True, True, True])
    ...
    def is_final_state(self):
        return self.refresh, self.position == self.goal_position and (self.points == 4 or self.lost_points == 4)

    def unique_hash(self):
        return str(self.position)

    def print_status(self):
        return ("Pobeda" +str(self.points)+"/"+str(self.lost_points)) if self.points > self.lost_points  else ("Gubitak" +str(self.points)+"/"+str(self.lost_points))
