...
class State(object):
    ...
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        if self.parent is None:  # ako nema roditeljsko stanje, onda je ovo inicijalno stanje
            ...
            self.all_boxes, self.all_boxes_checked = self.find_all_boxes(['b', 'y','br'])
...
class RobotState(State):
    ...
    def get_legal_positions(self):
        ...
        if self.game_is_finished():
            data = self.get_legal_moves_vertical(self.board, self.position, ['.', 'y', 'b', 'g'], ['br'], 1,
                                                 self.board.cols,
                                                 [True, True, True, True])
            data2 = self.get_legal_moves_diagonal(self.board, self.position, ['.', 'y', 'b', 'g'], ['br'], 1,
                                                  self.board.cols,
                                                  [True, True, True, True])
            new_positions = data
    ...
    def is_final_state(self):
        return self.position == self.goal_position and self.yellow_boxes >= 1

    def game_is_finished(self):
        return self.yellow_boxes >= 1
