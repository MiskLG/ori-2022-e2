...
class State(object):
    ...
    @abstractmethod
    def __init__(self, board, number_of_blue=0, parent=None, position=None, goal_position=None):
        """
    ...
    def get_next_states(self):
        ...
        for new_position in new_positions:
            next_state = self.__class__(self.board, self.number_of_blue, self, new_position, self.goal_position)
...
class RobotState(State):
    def __init__(self, board, number_of_blues=0, parent=None, position=None, goal_position=None):
        super(self.__class__, self).__init__(board,number_of_blues, parent, position, goal_position)
        row, col = self.position  # trenutno pozicija
        if self.board.data[row][col] == 'b' and self.number_of_blue == 0:
            self.eaten_boxes.append([row, col])
            self.number_of_blue += 1
    ...
    def is_final_state(self):
        if self.number_of_blue == 1:
            ...
            if self.board.data[row][col] == 'b':
                for eaten in self.eaten_boxes:
                    if eaten[0] == self.position[0] and eaten[1] == self.position[1]:
                        return True, self.position == self.goal_position
            return False, self.position == self.goal_position
        return False, False

    def unique_hash(self):
        return str(self.position)

    def get_number_of_blues(self):
        return self.number_of_blue
